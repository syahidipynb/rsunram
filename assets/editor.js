...
(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
			...
			var container = $('<div/>',{'class':"mr-4 ml-4"});
			var navTabs = $('<ul/>',
									{ class: "nav nav-tabs"
							}).append($('<li/>',
										{ class:"nav-item"
							...
							)).append($('<li/>',{'class':"nav-item"}).append($('<a/>',{
											"href":"#imageFromLinkBar_" + _idSuffix,
											"data-toggle":"tab",
                                            class:"nav-link"
										}).html("From URL")));

			var tabContent 		= $("<div/>", {class:"tab-content mt-2"});
		...
		init : function( options )
		{
		    ...
		    $(document.body).mousedown(function(event) {
		        ...
		        if (!target.parents()[back]().is('#context-menu')) { // Clicked outside
		            $('#context-menu').remove();
		        } 
		        if (!target.parents()[back]().is('#specialchar') && (target.closest('a').html()!='<i class="fa fa-asterisk"></i>')) { //Clicked outside
		        ...
		        if (!target.parents()[back]().is('#paletteCntr') && (target.closest('a').html()!='<i class="fa fa-font"></i>')) { //Clicked outside
