                ...
                <div class="x_panel">
        	        <div class="x_title">
						...
						<form id="submit">
							...
							<div class="form-group">
								<button class="btn btn-success" id="btn_save" type="submit">Upload</button>
							</div>
						</form>	  
...
<div class="modal fade docs-cropped" id="Modal_Edit" aria-hidden="true" aria-labelledby="getCroppedCanvasTitle" role="dialog" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">                                              
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" >Silahkan masukkan nama fasilitas yang baru</h4>
            </div>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9 col-sm-9 col-xs-12">
                      <textarea name="NAMAS_edit" id="NAMAS_edit" class="resizable_textarea form-control" placeholder="Nama Fasilitas" ></textarea>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		function show_asside(){
		    $.ajax({
		        type  : 'ajax',
		        url   : '<?php echo site_url('Fasilitas_controllers/asside_data')?>',
		        async : false,
		        dataType : 'json',
                        ...
                        html += '<div class="col-md-55">'+
                        			'<div class="">'+
                          				'<div class="image view view-first">'+
                            				'<img style="width: 100%; display: block;" src="<?php echo base_url();?>assets/images/Fasilitas/'+data[i].Photo+'" alt="image" />'+
                            				'<div class="mask">'+
                              					'<p>'+data[i].Caption+'</p>'+
                            				'</div>'+
                                        '</div>'+
		...
		$('#submit').submit(function(e){
		    e.preventDefault();
		        $.ajax({
		            url  : "<?php echo site_url('Fasilitas_controllers/uploads')?>",
		            type:"post",	
        ...
        $('#btn_delete').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url  : "<?php echo site_url('Fasilitas_controllers/delete')?>",
                dataType : "JSON",
                data : {id:id},
                success: function(data){
        ...
        $('#btn_update').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url  : "<?php echo site_url('Fasilitas_controllers/update')?>",
                dataType : "JSON",
                data : {kd:kd , nm:nm},
                success: function(data){
                    $('[name="KODES_edit"]').val("");
